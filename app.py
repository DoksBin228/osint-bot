...
class SimpleDB:
    ...
    def save_person(self, data: Dict):
        ...
        with self.conn.cursor() as cur:
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (data.get('full_name'), data.get('phone'), data.get('email'), data.get('address'), json.dumps(data.get('social_links', {})), data.get('risk_score', 0)))

    def search_people(self, query: str) -> List[Dict]:
        if not self.conn:
            return []
        with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT * FROM people WHERE full_name ILIKE %s OR phone ILIKE %s OR email ILIKE %s ORDER BY risk_score DESC", (f'%{query}%', f'%{query}%', f'%{query}%'))
            return cur.fetchall()
    ...
    def reset_user_limits(self, user_id: int):
        if not self.conn:
            return
        with self.conn.cursor() as cur:
            cur.execute("INSERT INTO user_limits (user_id, request_count, last_reset) VALUES (%s, 0, CURRENT_DATE) ON CONFLICT (user_id) DO UPDATE SET request_count = 0, last_reset = CURRENT_DATE", (user_id,))

    def increment_request(self, user_id: int):
        if not self.conn:
            return
        with self.conn.cursor() as cur:
            cur.execute("INSERT INTO user_limits (user_id, request_count, last_reset) VALUES (%s, 1, CURRENT_DATE) ON CONFLICT (user_id) DO UPDATE SET request_count = user_limits.request_count + 1", (user_id,))
...
def process_search(message, user_id):
    ...
    if db.conn:
        ...
        rem = "∞"
    results = {'full_name': query if not query.startswith('+') and '@' not in query else '', 'phone': query if query.startswith('+') else '', 'email': query if '@' in query else '', 'address': '', 'social_links': {}, 'risk_score': 0}
    if db.conn:
...
def reset_limit(message):
    if db.conn:
        ...
        bot.send_message(message.chat.id, f"{header('ЛИМИТ ОБНОВЛЁН')}\n\n⚠️ БД не настроена, лимит бесконечный\n\n{footer()}")
...
if __name__ == "__main__":
    bot.remove_webhook()
    bot.set_webhook(url=f"https://{os.environ.get('RENDER_EXTERNAL_HOSTNAME', 'localhost')}/")
